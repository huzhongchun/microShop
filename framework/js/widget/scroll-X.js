 ...
 * ------ {bool} 需要另外计算scroll－box的宽度，内部的内容可能不是规则的宽度
 * 
 */
    ...
    scrollXObject.prototype = {
        ...
        init: function() {
            ...
            console.log(opt.dom);
            if (!opt.dom || opt.dom.length == 0) {
                console.log('dom为空!');
                return false;
            }
            self.dom = opt.dom.last();
            var childs = self.dom.children();
            for (var i = 0; i < childs.length; i++) {
                //获取元素的padding、border,不允许使用margin, 左边元素的marginRight和右边元素的marginLeft会合并取最大的，
                //判断盒子模型,安卓机器上盒子模型表现不一
                var domCss = window.getComputedStyle(childs[i],null);

                if(domCss.boxSizing == 'content-box'){
                    var borderWidth = parseFloat(domCss.borderRightWidth)+parseFloat(domCss.borderLeftWidth);
                    self.boxWidth +=$(childs[i]).width()+borderWidth;
                }
                else if(domCss.boxSizing == 'border-box'){
                    var paddingWidth = parseFloat(domCss.paddingRight)+parseFloat(domCss.paddingLeft);
                    var borderWidth = parseFloat(domCss.borderRightWidth)+parseFloat(domCss.borderLeftWidth);
                    self.boxWidth +=$(childs[i]).width()+paddingWidth+borderWidth;
                }
                
            }
            self.dom.css('visibility','visible');
            $(this.dom).width(self.boxWidth);
        ...
        getMaxMove: function() {
            var self = this, opt = this.settings;
            var width = $(self.dom).width();         /*用this.id及时更新dom的状态*/
