...
fairObject.prototype = {
    ...
    platform: function(){
      var ua = navigator.userAgent;
      if(ua.toLowerCase().match(/taoo/) && ua.toLowerCase().match(/android/)){
        return 'appAndroid';
      }else if(ua.toLowerCase().match(/taoo/) && ua.toLowerCase().match(/iphone|ipad/)){
        return 'appIos';
      }else{
        return 'browser';
      }
    ...
    //添加谷歌的事件统计
    //category: (string)事件对象，事件触发所在的范围，如在某个页面下
    //action: (string)事件行为，‘click’ 等
    //label: (string)事件标识，某次事件触发唯一标识
    //value: 与事件相关的数值，必须为整型的数字
    addEventGa: function(category,action,label,value){
      if(ga){
...
;(function($){
  ...
  $.ajax = function(options){
    ...
    if (dataType == 'jsonp' || hasPlaceholder) {
      //后端的设置是响应__callback   2015-11-25
      if (!hasPlaceholder) settings.url = appendQuery(settings.url, '__callback=?')
      //正常的
...
!(function($) {
   ...
   $.setCookie = function(name, value, expires, path, domain, secure) {
        ...
        // 去掉escape, 避免escape 把＋解析成空格   ---20160428
    ...
    $.setUrlQueryString = function(url,name,str) {
        //获取url中的参数,并且替换
        var reg = new RegExp('&'+name + '=([^&#]*)');
        var r = url.match(name);
        if (r != null) {
            return  url.replace(reg,'&'+name+'='+str);
        } else {
            if(url.match(/\?/))
                return url+'&'+name+'='+str;
            else
    ...
    $.fn.urlScheme = function(options) {
      var country = options && options.country ? options.country : 'cn';
      var className = options && options.className ? options.className : '';
      if(F.platform() != 'appIos' ){

        return this.each(function(){
          var href = $(this).attr('href');
          if(href && href.match(/page_label=note_detail/)){
              var noteId = href.match(/note_id=(\d+)/) ? href.match(/note_id=(\d+)/)[1] : '';
              if(noteId)
                href = '/mobile/touch/note_detail?note_id='+noteId+'&__c='+country;
          }else if(href && href.match(/page_label=magazine_detail/)){
              var noteId = href.match(/magazine_id=(\d+)/) ? href.match(/magazine_id=(\d+)/)[1] : '';
              if(noteId)
                href = '/mobile/touch/magazine_detail?magazine_id='+noteId+'&__c='+country;
          }else if(href && href.match(/page_label=collection_detail/)){
              var noteId = href.match(/collection_id=(\d+)/) ? href.match(/collection_id=(\d+)/)[1] : '';
              if(noteId)
                href = '/mobile/touch/collection_detail?collection_id='+noteId+'&__c='+country;
          }
          //个人中心
          else if(href && href.match(/page_label=mine/)){
              var uId = href.match(/uid=(\d+)/) ? href.match(/uid=(\d+)/)[1] : '';
              if(uId)
                href = '/mobile/touch/user_center?u_id='+uId+'&__c='+country;
          }
          else if(href && href.match(/page\/user\/mine/)){
              var uId = href.match(/uid=(\d+)/) ? href.match(/uid=(\d+)/)[1] : '';
              if(uId)
                href = '/mobile/touch/user_center?u_id='+uId+'&__c='+country;
          }
          else if(href && href.match(/page\/user\/profile/)){
              var uId = href.match(/uid=(\d+)/) ? href.match(/uid=(\d+)/)[1] : '';
              if(uId)
                href = '/mobile/touch/user_center?u_id='+uId+'&__c='+country;
          }
          else if(href && href.match(/page_label=profile/)){
              var uId = href.match(/uid=(\d+)/) ? href.match(/uid=(\d+)/)[1] : '';
              if(uId)
                href = '/mobile/touch/user_center?u_id='+uId+'&__c='+country;
          }
          else if(href && href.match(/^taoo:\/\/page\/user\/mine/)){
              var uId = href.match(/uid=(\d+)/) ? href.match(/uid=(\d+)/)[1] : '';
              if(uId)
                href = '/mobile/touch/user_center?u_id='+uId+'&__c='+country;
          }
          else if(href && href.match(/^taoo:\/\/page\/user\/profile/)){
              var uId = href.match(/uid=(\d+)/) ? href.match(/uid=(\d+)/)[1] : '';
              if(uId)
                href = '/mobile/touch/user_center?u_id='+uId+'&__c='+country;
          }
          //用户粉丝列表
          else if(href && href.match(/^taoo:\/\/page\/fans_user_list\/user/)){
              var dataUid = $(this).attr('data-uid');
              var hrefUid = href.match(/uid=(\d+)/) ? href.match(/uid=(\d+)/)[1] : '';
              var uId = dataUid ? dataUid : hrefUid;
              if(uId)
                href = '/mobile/touch/users_list?u_id='+uId+'&__t=fans&__c='+country;
          }
          //用户关注列表
          else if(href && href.match(/^taoo:\/\/page\/my_favorite\/user/)){
              var dataUid = $(this).attr('data-uid');
              var hrefUid = href.match(/uid=(\d+)/) ? href.match(/uid=(\d+)/)[1] : '';
              var uId = dataUid ? dataUid : hrefUid;
              if(uId)
                href = '/mobile/touch/users_list?u_id='+uId+'&__t=follows&__c='+country;
          }
          //笔记点赞列表
          else if(href && href.match(/^taoo:\/\/page\/favorite_user_list\/note/)){
              var noteId = href.match(/note_id=(\d+)/) ? href.match(/note_id=(\d+)/)[1] : '';
              if(noteId)
                href = '/mobile/touch/users_list?note_id='+noteId+'&__t=note_favorite_list&__c='+country;
          }
          //搜索页
          else if(href && href.match(/page_label=search_data/)){
              var params = href.match(/\?(.+)/)[1];
              href = '/mobile/touch/search_list?__c='+country+'&'+params;
          }
          //“更多”的 卡片落地页
          else if(href && href.match(/^taoo:\/\/page\/card\?page_id=/)){
              var pageId = href.match(/page_id=(\d+)/) ? href.match(/page_id=(\d+)/)[1] : '';
              var params = href.match(/\?(.+)/)[1];
              href = '/mobile/touch/page_card?__c='+country+'&'+params;
          }
          if(className){
